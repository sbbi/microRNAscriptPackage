...
class Main():
    def __init__(self):
        ...
        ranking = []
        with open("step3/%s/tools/%s.all.libsvm.fscore"%(kingdomSelection, kingdomSelection)) as f:
            for lin in f:
                line = lin.strip()
                sp = line.split(':')
        ...
        for i in gen:
            without = ranking[i:]
            without = set(without)
            convert_to_libsvm_format(training_set[0], 'step3/%s/training%03d.libsvm'%(kingdomSelection,i), without)
            convert_to_libsvm_format(testing_set[0], 'step3/%s/testing%03d.libsvm'%(kingdomSelection,i), without)
...
def generateTrainingAndTestingSets(positive_samples, negative_samples):
    ...
    for i in xrange(len(positive_samples)):
        print "loop %s" % i
        half = int(math.ceil(len(positive_samples[0])*.7))
